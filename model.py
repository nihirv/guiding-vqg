...
class VQGModel(nn.Module):
    ...
    def forward(self, images, question_ids, question_attention_masks, input_ids, input_attention_masks, obj_features, obj_locations):
        ...
        src_mask = torch.cat((encoder_attention_mask, object_mask), dim=1)
        ...
        multi_encoder_hidden_states = multi_encoder_outputs.last_hidden_state

        kld = None
        if self.latent_transformer:
            multi_encoder_hidden_states, kld = self.latent(multi_encoder_hidden_states)
        ...
        outputs = self.decoder(input_ids=question_ids, labels=question_ids, attention_mask=question_attention_masks,
                               encoder_hidden_states=multi_encoder_hidden_states, encoder_attention_mask=src_mask)
        loss = outputs["loss"]
    ...
    def decode_greedy(self, images, input_ids, input_attention_masks, obj_features, obj_locations, max_decode_len=50):
        ...
        if self.latent_transformer:
            multi_encoder_hidden_states, kld = self.latent(multi_encoder_hidden_states)
            # z, _ = self.latent(encoder_hidden_states[:, 0])
            # encoder_hidden_states[:, 0] = encoder_hidden_states[:, 0] + z
